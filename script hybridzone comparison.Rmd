...
leaftraits<-leaftraits[leaftraits$vialID%in%env.2014$vialID,]
...
leaftraits<-na.omit(leaftraits)

#select subset traits to prevent collinearity
leaftraits<-merge(leaftraits, env[,c(1,10)], by="vialID")
rownames(leaftraits)<-leaftraits$vialID
leaftraits<-leaftraits[,c(3,4,6,7,10,11)]
colnames(leaftraits)<-c("percN", "percP", "meanSLA", "meanWatercontent", "CN", "morphotype")
...
#env$morphotype<-factor(env$morphotype,levels(env$morphotype))

################
#  make frequency matrix of species by site; ie instead of absolute numbers put in numbers proportionate to total site abundance. 
# prob with frequency is gives diff picture from fourth corner absolute numbers

#hem.freq<-hem.new/rowSums(hem.new)
#rownames(hem.freq)<-rownames(hem.new)
...
ordiellipse(p,groups=env.both$site,draw="polygon",col="grey90",label=T)
```
<br>
...
adonis(hem.new~leaftraits$morphotype+leaftraits$percN+leaftraits$meanWatercontent+leaftraits$meanSLA+leaftraits$percP, by="margin")


adonis(hem.new~env.subset$tree.height+leaftraits$morphotype+leaftraits$meanSLA+leaftraits$percN+env.subset$tree.height+env.subset$site, by="margin")



mod.leaf<-cca(hem.new~leaftraits$percN+leaftraits$morphotype+leaftraits$meanSLA+leaftraits$meanWatercontent+leaftraits$percP)
mod.leaf.age<-cca(hem.new~leaftraits$percN+leaftraits$morphotype+leaftraits$meanSLA+env.subset$age)
mod.leaf.tree<-cca(hem.new~leaftraits$percN+leaftraits$morphotype+leaftraits$meanSLA+env.subset$tree.height)
mod.leaf.age.site<-cca(hem.new~leaftraits$percN+leaftraits$morphotype+leaftraits$meanSLA+env.subset$tree.height+env.subset$age+env.subset$site)
mod.leaf.site<-cca(hem.new~leaftraits$percN+leaftraits$morphotype+leaftraits$meanSLA+env.subset$site+leaftraits$meanWatercontent+leaftraits$percP)
...
vare.cca <- cca(decostand(hem.new, "total") ~ ., data = leaftraits[,c(1,6)]) 
...
nymph.morpho[10,1]<-
mean(rowSums(hem.2014[ rownames(hem.2014)[leaftraits$morphotype=="H"]  ,rownames(traits)[traits$location_nymph=="shelter"] ]))
nymph.morpho[11,1]<-
mean(rowSums(hem.2014[ rownames(hem.2014)[leaftraits$morphotype=="P"]  ,rownames(traits)[traits$location_nymph=="shelter"] ]))
nymph.morpho[12,1]<-
mean(rowSums(hem.2014[ rownames(hem.2014)[leaftraits$morphotype=="G"]  ,rownames(traits)[traits$location_nymph=="shelter"] ]))
...
ggplot(data=leaftraits, aes(x=site, y=percN, color=morphotype)) +
    geom_point()
ggplot(data=leaftraits, aes(x=site, y=percP, color=morphotype)) +
    geom_point()
ggplot(data=leaftraits, aes(x=site, y=meanSLA, color=morphotype)) +
    geom_point()
ggplot(data=leaftraits, aes(x=site, y=meanWatercontent, color=morphotype)) +
    geom_point()

### perc N

anova(lm(leaftraits$percN~leaftraits$morphotype*leaftraits$site))
TukeyHSD(aov(leaftraits$percN~leaftraits$morphotype*leaftraits$site))

anova(lm(leaftraits$percN~leaftraits$site))
TukeyHSD(aov(leaftraits$percN~leaftraits$site))
...
anova(lm(leaftraits$percP~leaftraits$site))
TukeyHSD(aov(leaftraits$percP~leaftraits$site))
...
anova(lm(leaftraits$meanSLA~leaftraits$site))
TukeyHSD(aov(leaftraits$meanSLA~leaftraits$site))
...
anova(lm(leaftraits$meanWatercontent~leaftraits$site))
TukeyHSD(aov(meanWatercontent$meanSLA~leaftraits$site))
...
adonis(hem[rownames(hem) %in% leaftraits$vialID,] ~  leaftraits$percN +leaftraits$percP+leaftraits$morphotype  + leaftraits$meanWatercontent+leaftraits$meanSLA+leaftraits$site , by="terms")
...
anova(lm(rowSums(hem.big)~env.big$morphotype))
TukeyHSD(aov(rowSums(hem.big)~env.big$morphotype))
# p<0.05
...
tem<-1/(1-ChaoShannon(as.data.frame(t(hem.both)) )[,2])
tem[26]<-1 
anova(lm(tem~env.both$morphotype))
...
plot(diversity(hem.big)/log(specnumber(hem.big))~env.big$morphotype, ylab="Pielou's Evenness")
```
...
```{r}
mod.leaf.site<-cca(hem[rownames(hem)%in%temp5$vialID,]~temp5$percN+temp5$morphotype+temp5$meanSLA+temp5$meanWatercontent+temp5$percP+temp5$site)


mod.leaf.site
# 56% of variation constrained

mod.leaf<-cca(hem[rownames(hem)%in%temp5$vialID,]~temp5$percN+temp5$morphotype+temp5$meanSLA+temp5$meanWatercontent+temp5$percP)
mod.leaf
...
temp5<-temp5[order(rownames(temp5)),]
hem.2014<-hem.2014[order(rownames(hem.2014)),]
hem.2014<-hem.2014[,colnames(hem.2014)%in%rownames(traits)]

mod<-manyglm(mvabund(hem.2014)~leaftraits$percN+leaftraits$percP+leaftraits$morphotype+leaftraits$meanWatercontent+leaftraits$meanSLA, family="negative.binomial"+(1|leaftraits$site))
...
```{r}
ftspp1=traitglm(hem.sub,temp5,method="glm1path", family="negative.binomial")
...
hem.sub<-hem.sub[,colnames(hem.sub)%in%rownames(traits)]
...
model4th<-traitglm(hem.2014, leaftraits[,c(3,4,5,6,8)], traits,  method="manyglm", family="negative.binomial")
...
ftSmall=traitglm(hem.sub,temp5[,c(3,4,5,6,8)],traits)
anova(ftSmall)
# does permutations = work for this function?

model4th<-traitglm(hem.test, leaftraits, traits.test,  method="glm1path", family="negative.binomial")
model4th$fourth
